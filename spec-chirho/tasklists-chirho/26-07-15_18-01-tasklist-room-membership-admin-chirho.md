...
- [x] **A1. `RemoveRequestChirho`** struct: `{ from_session_chirho, from_agent_chirho, session_chirho,
  agent_chirho, room_chirho }` (serde, all validated with `validate_token_chirho`).
- [x] **A2. `remove_agent_chirho(conn, request) -> Result<Value, String>`**:
...
- [x] **A3. Route:** add `("POST", "/v1/remove_chirho")` arm in `route_http_chirho` (mirror
  `/v1/register_chirho`).
- [x] **A4. CLI:** add `"remove" => run_remove_cli_chirho(&args_chirho)` and implement
...
- [x] **A5. Add-notify:** extend register so the add path can nudge the newcomer **without** a second
...
- [x] **A6. Tests** (`#[cfg(test)]` in `main.rs`, in-memory SQLite like `in_memory_db_registers_agent_and_room_chirho`):
...
- [x] **B1. Mouse capture lifecycle.** Add `EnableMouseCapture` to the `execute!` after
...
- [x] **B2. Modal state.** Add `mode_chirho: TuiModeChirho` to `TuiStateChirho`:
  - `NormalChirho`
...
- [x] **B3. Hit-testing.** Ratatui has no built-in hit-testing — record the rendered geometry during
  ...
  handler agree.
- [x] **B4. Mouse events** in `handle_event_chirho` (`CrosstermEventChirho::Mouse`):
...
- [x] **B5. Popup rendering.** Centered overlay via `Clear` + a small bordered `Block`:
...
- [x] **B6. Keyboard parity (because "mouse + keyboard both").** The compose box currently eats all
...
- [x] **B7. Wire actions to the broker.** On confirm-remove → POST `/v1/remove_chirho` (from = the TUI's
  ...
  `needs_render_chirho`.
- [x] **B8. Tests:** add-form field → `(identity, tmux_target)` derivation incl. name normalization;
  hit-test row math (`agent_index_at`); mode transitions (right-click→menu→confirm→Normal).

## Part C — Docs & housekeeping

- [x] **C1.** Update the **TUI commands** sections of `README.md` and `SKILL.md` with the new controls
  (right-click / `[ + add ]` / `Tab` focus / `Up`·`Down` / `x` remove / `a` add / `Esc`).
- [x] **C2.** Add a small mermaid DAG for the add/remove membership flow under
  `spec-chirho/workflows-chirho/` and comment the participating functions to point at it (repo convention).
- [x] **C3.** If `tui_chirho.rs` heads past ~1k lines (it's ~640 now; the modal machine + mouse handling
...
- [x] `cargo build --release` clean; `cargo clippy --release -- -D warnings` clean (fix, don't suppress).
- [x] `cargo test --release` green (existing 10 + the new Part A/B tests).
- [x] Manual: right-click remove notifies + unsubscribes (this room only, verified in SQLite); `+`/`a`
...
- [x] John 3:16 header on any new file; **Chirho suffix** on every new identifier (correct casing);
  files/dirs within size limits.
- [x] Docs updated (C1). Commit only the files you changed, by name — do not sweep my uncommitted work.
